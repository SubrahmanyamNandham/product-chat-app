...
async function runTests() {
  ...
  // Login all users
  const agentToken = await login('agent@test.com', 'password123');
  ...
  for (const c of allAgentConvs) {
    const productName = (c as Record<string, unknown>).productId && typeof (c as Record<string, unknown>).productId === 'object' 
      ? ((c as Record<string, unknown>).productId as Record<string, unknown>).name 
      : String((c as Record<string, unknown>).productId);
